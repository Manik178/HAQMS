...
router.post('/checkin', authenticate, async (req, res) => {
  try {
    ...
    // Use a serializable transaction to prevent race conditions on token numbers
    const newToken = await prisma.$transaction(async (tx) => {
      // 1. Read current max token inside transaction
      ...
      // 2. Create new token inside the same transaction
